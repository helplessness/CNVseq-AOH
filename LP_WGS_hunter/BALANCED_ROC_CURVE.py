...
def show_info(filename, info, pairs):
    ...
    ###########################################
    print('Ancestral makeup: %s, Fraction of alleles matched to the reference panel: %.3f.' % (ancestral_makeup, info['statistics']['matched_alleles']['EAS']))

    ###########################################
    # for pair in pairs:
    if 'LLRs_per_chromosome' in S and tuple(pairs) in S['LLRs_per_chromosome']:
        L = S['LLRs_per_chromosome'][tuple(pairs)]
        print(f"--- Chromosome-wide LLR between {pairs[0]:s} and {pairs[1]:s} ----")
...
def binning(LLRs_per_window,info,num_of_bins):
    ...
    bins = bin_genomic_windows(list_of_windows, 'chr'+info['chr_id'], num_of_bins)
    X = [*bins]
...
def collect_data(criteria, num_of_bins, work_dir):
    ...
    ####simulated
    filenames = glob.glob(work_dir.rstrip('/') +'/'+ "*LLR.p*")
    ...
    for filename in filenames:
        try:
            ...
            #############################
            # subinfo = {x: info.get(x,None) for x in criteria}
            subinfo = {'scenarios': ['BPH', 'SPH']}
            scenario_all =['BPH', 'SPH']
...
def prediction_rates(data, thresholds, positive='both'):
    ...
    # print(prediction_rates['chr10'][(0.0, 0.06666666666666667)][-6.96969696969697])
