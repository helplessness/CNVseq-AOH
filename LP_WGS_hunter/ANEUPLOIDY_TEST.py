...
from LP_WGS_hunter.HOMOGENOUES_MODELS import homogeneous
from LP_WGS_hunter.RECENT_ADMIXTURE_MODELS import recent_admixture
from LP_WGS_hunter.DISTANT_ADMIXTURE_MODELS import distant_admixture
...
class supporting_dictionaries:
    ...
    def build_score_dict(self, reads_dict, hap_tab_per_group, number_of_haplotypes_per_group, ancestral_makeup, min_HF):
        ...
        #####  添加  样本深度限定,质量太好的数 采样过大  限定 满足freq的深度为22
        score_dict = {}
        ...
        for read_id in reads_dict:
            ...
            if len(haplotypes):
                #####   这里设置每个位点的深度 最多是22 
                cache = [(a * popcount(reduce(and_,hap)) for hap in product(*haplotypes[group2][:22]))
        ...
        LLRs = {(i,j):
                {window: tuple(LLR(attrgetter(i)(l), attrgetter(j)(l)) for l in likelihoods_in_window)
                           for window,likelihoods_in_window in likelihoods.items()}
                                                            for i,j in pairs}
        # print(LLRs[('BPH','SPH')])
        # print(mean_and_std_of_mean_of_rnd_var(LLRs[('BPH','SPH')]))
...
def aneuploidy_test(obs_filename,leg_filename,hap_filename,
    ...
    models_filename = kwargs.get('model', os.path.join(path, ('MODELS12.p' if max_reads>8 else ('MODELS8.p' if max_reads>4 else 'MODELS4.p'))))
    if not os.path.exists(models_filename):
        print('models path', models_filename)
        raise('ERROR: The models file does not exist. Please check the path to the models file.')
    ...
    ####添加
    ancestral_makeup = set(ancestral_makeup)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-OD', '--output-dir', type=str, metavar='output_dir',
